...
class FForm(Form):
   f=RadioField('THE Item',choices=[('0',u'安全策略'),('1',u'协商周期')],default='0')
...
def ch():
    ...
    if form.validate_on_submit():
       ...
       if ch=='1':
         ...
         form.ch.data=''
         return redirect('/out')
    return render_template('mode.html',form=form,ch=ch)
...
def XS():
    ...
    if form.validate_on_submit():
       ...
       form.xs.data=''
       return redirect('/out')
    return render_template('xieshang.html',form=form,xs=xs)
